...
export async function handler(event) {
  ...
  // Handle FireHOL blocklist fetch
  try {
    const res = await fetchWithRetry("https://iplists.firehol.org/files/firehol_level1.netset");
    console.log("FireHOL fetch status:", res.status);
    if (!res.ok) throw new Error(`HTTP error: ${res.status}`);
    const text = await res.text();
    const lines = text.split("\n").filter(line => line && !line.startsWith("#"));
    const feed = lines.slice(0, 50).map(line => ({
      ipAddress: line.trim(),
      status: "Malicious",
      source: "FireHOL"
    }));
    console.log("FireHOL feed parsed:", feed.length, "entries");

    if (feed.length === 0) {
      console.warn("No data in FireHOL response");
      return {
        statusCode: 200,
        body: JSON.stringify({ error: "No data returned from FireHOL blocklist" })
      };
    }
    ...
    console.error("FireHOL fetch error:", err);
    return {
      statusCode: 500,
      body: JSON.stringify({ error: `Failed to fetch FireHOL blocklist: ${err.message}` })
    };
  }
}
